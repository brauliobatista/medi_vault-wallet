...
**All comments in `.sql` and `.md` files (`--`, `%%`) must be written in English**, even though the seed data values themselves (names, addresses, country names, etc.) are Portuguese. This applies to section headers, inline notes, and everything in `database/*.sql` and `database/erd.md`. Do not write new Portuguese comments — only the Confluence page content and the seed data values stay in Portuguese.
...
## Fixed enum vs. config table

Two patterns exist for a column with a bounded set of values — pick based on whether the list is closed or expected to grow:
